...
def load_resized_hsv_img(fname, shortest_length=30.):
    ...
    img_orig = cv2.cvtColor(np.asarray(fname), cv2.COLOR_RGB2BGR)
    # img_orig = fname.convert('RGB')
...
def orig_score(fpath, img_score_size=30., img_ret_size=300.):
    img_hsv, img_size = load_resized_hsv_img(fpath, shortest_length=img_score_size)
    high_probability_type = "同类色"
    high_probability= 0
    for t_type in templates.keys():
        ...
        for alpha in range(0, 360):
            ...
            if alpha == 0:
                score_orig = score
                probability_orig = 100-score_orig
        if probability_orig > high_probability:
            high_probability_type = t_type
            high_probability = probability_orig
        # print("{:s}, {:.2f}%".format(t_type, probability_orig))

    return high_probability_type, high_probability
...
if __name__ == "__main__":
    ...
    high_probability_type, high_probability = orig_score("/Users/lijialong/Desktop/6.7289.jpg")
    # print("最可能色环搭配:", high_probability_type)
    # print("概率:", high_probability)
    # # H * 2 = degree [0, 360]
    # # Source: https://docs.opencv.org/trunk/de/d25/imgproc_color_conversions.html#color_convert_rgb_hsv
    # h, w, _ = img_orig.shape
    # shortest_length = 30.
