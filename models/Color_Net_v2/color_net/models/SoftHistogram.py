...
class SoftHistogram(nn.Module):
    ...
    def __init__(self, n_features, n_examples, num_bins, quantiles=False):
        ...
        self.bin_centers_conv = nn.Conv1d(self.in_channels, self.num_bins * self.in_channels, kernel_size=1, bias=True)
        # print(self.in_channels, self.bin_centers_conv)
        ...
        self.hist_pool = nn.AvgPool1d(kernel_size=81, stride=81, padding=0, ceil_mode=False)
    ...
    def forward(self, input):
        ...
        input = F.interpolate(input, size=(81, 81), mode="bilinear", align_corners=False)

        bn, c, h, w = input.shape
        input = self.bin_centers_conv(input.view(bn, c, h*w))
        ...
        # 为什么要flatten() input.flatten()
        # print(input)
        return input.view(bn, -1)
...
class SoftHistogramRBF(nn.Module):
    ...
    def __init__(self, n_features, n_examples, num_bins, quantiles=False):
        ...
        self.bin_widths_conv = nn.Conv1d(self.num_bins * self.in_channels, self.num_bins * self.in_channels,
                                         kernel_size=1, groups=self.num_bins * self.in_channels, bias=False)
